...
class HttpSession(BaseClient,requests.Session):
    """
    ...
    def __init__(self, base_url, *args, **kwargs):
        protocol_ok =  base_url.startswith("http://") or  base_url.startswith("https://")
        if not protocol_ok: self.base_url = "http://" + base_url
...
class HttpsSession(HttpSession):
    """
    Simple shim client that deals with adding "https:// as needed.
    this handles --client_type https on the command line.
    """
    def __init__(self, base_url, *args, **kwargs):
